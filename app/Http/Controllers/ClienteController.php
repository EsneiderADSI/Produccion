...
protected function obtenerUnaNovedad($id)
	...
	protected function almacenarNovedad(Request $request)
    {
        ...
        $respuesta = $this->realizarPeticion('POST', "http://agroproduccion.com:81/productos/{$producto_id}/novedades", ['headers' => ['Authorization' => $accessToken], 'form_params' => $request->all()]);
    }
...
protected function almacenarSuelo(Request $request)
 ...
 //Métodos para Cultivos
		...
		protected function obtenerTodosLosCultivos()
	{
		$respuesta = $this->realizarPeticion('GET', 'http://agroproduccion.com:81/cultivos');
		$datos = json_decode($respuesta);
		$cultivo = $datos->data;
		return $cultivo;
		}
protected function obtenerUnCultivo($id)
    {
        $respuesta = $this->realizarPeticion('GET', "http://agroproduccion.com:81/cultivos/{$id}");
        $datos = json_decode($respuesta);
        $cultivo = $datos->data;
        return $cultivo;
    }
...
protected function almacenarCultivo(Request $request)
    {
    	$accessToken = 'Bearer ' . $this->obtenerAccessToken();
        $respuesta = $this->realizarPeticion('POST', 'http://agroproduccion.com:81/cultivos', ['headers' => ['Authorization' => $accessToken], 'form_params' => $request->all()]);
    }
    protected function modificarCultivo(Request $request)
    {
    	$accessToken = 'Bearer ' . $this->obtenerAccessToken();
        $id = $request->get('id');
        $respuesta = $this->realizarPeticion('PUT', "http://agroproduccion.com:81/cultivos/{$id}", ['headers' => ['Authorization' => $accessToken], 'form_params' => $request->except('id')]);
    }
    protected function removerCultivo(Request $request)
    {
    	$accessToken = 'Bearer ' . $this->obtenerAccessToken();
        $id = $request->get('cultivo_id');
        $respuesta = $this->realizarPeticion('DELETE', "http://agroproduccion.com:81/cultivos/{$id}", ['headers' => ['Authorization' => $accessToken]]);
    }
